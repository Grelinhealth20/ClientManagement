...
/**
 * Document upload widget — a clearly-bordered card with a distinct drag-and-drop
 * zone, uploaded-file rows (view + delete), and live upload progress. Uploads go
 * straight to S3 via /api/onboarding/documents.
 */
  ...
  async function uploadFiles(list) {
    ...
    try {
      for (const file of arr) {
        // 1) Ask our server for a presigned URL (validates + builds the S3 key).
        const presign = await fetch("/api/onboarding/documents/presign", {
          method: "POST",
        ...
        // 2) Upload the bytes straight to S3 (no serverless body limit).
        const put = await fetch(p.url, { method: "PUT", headers: p.headers, body: file });
        if (!put.ok) throw new Error("Upload to storage failed.");

        // 3) Confirm — server verifies the object and records it.
        const confirm = await fetch("/api/onboarding/documents/confirm", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "same-origin",
          body: JSON.stringify({
  ...
  return (
    <div
      className={`flex flex-col rounded-2xl border-2 bg-white p-3.5 transition-all duration-200 ${
        dragging
          ? "border-copper shadow-[0_0_0_4px_rgba(207,148,85,0.15)]"
          : has
          ? "border-emerald-300"
          : "border-slate-200 hover:border-copper/40"
      }`}
    >
      {/* Header */}
        ...
        <div className="flex items-start gap-2">
          <span
            className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ${
              has ? "bg-emerald-100 text-emerald-700" : "bg-navy/[0.06] text-copper-700"
            }`}
          >
          ...
          <p className="text-[12px] font-bold leading-snug text-navy">
            {label}
      ...
      <AnimatePresence initial={false}>
        {files.map((f) => (
          <motion.div
            ...
            className="mb-2 flex items-center justify-between gap-2 rounded-xl border border-emerald-200 bg-emerald-50/70 px-2.5 py-2"
          >
            <button type="button" onClick={() => view(f)} className="flex min-w-0 items-center gap-2 text-left">
              ...
              <span className="min-w-0">
                <span className="block truncate text-[12px] font-bold text-navy">{f.filename}</span>
                <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600">
                  {formatBytes(f.size_bytes)}
                  <span className="text-slate-300">·</span>
                  <span className="underline decoration-emerald-300">View</span>
                </span>
              </span>
            </button>
            <button
              ...
              className="grid h-7 w-7 shrink-0 place-items-center rounded-lg border border-transparent text-slate-400 transition-colors hover:border-rose-200 hover:bg-white hover:text-rose-600 disabled:opacity-50"
            >
              {removingId === f.id ? (
                <Spinner />
              ) : (
      ...
      {/* Drag-and-drop zone — clearly bordered */}
      <button
        type="button"
        ...
        className={`group/zone relative mt-auto flex w-full flex-col items-center justify-center gap-2 overflow-hidden rounded-xl border-2 border-dashed px-4 py-5 text-center transition-all duration-200 ${
          dragging
            ? "scale-[1.01] border-copper bg-copper/10"
            : "border-slate-300 bg-slate-50/70 hover:border-copper/60 hover:bg-copper/5"
        }`}
      >
        {busy ? (
            ...
            <motion.span
              ...
              className={`grid h-11 w-11 place-items-center rounded-full transition-colors ${
                dragging
                  ...
                  : "bg-white text-slate-400 shadow-sm ring-1 ring-slate-200 group-hover/zone:text-copper-700 group-hover/zone:ring-copper/30"
              }`}
            >
